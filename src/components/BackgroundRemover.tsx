...
export default function BackgroundRemover() {
  ...
  // Shared model-load status (download happens once for the whole session).
  const [phase, setPhase] = useState<BgProgress | null>(null);
  ...
  useEffect(() => {
    setSupported(backgroundRemovalSupported());
  }, []);
  ...
  // Sequential worker — keeps memory predictable on every device.
  const pump = useCallback(async () => {
    ...
    try {
      for (;;) {
        ...
        try {
          const res = await removeBackground(next.file, (p) => {
          ...
          const msg =
            e instanceof Error
              ? e.message
              : "Something went wrong removing the background.";
  ...
  const addFiles = useCallback(
    (list: FileList | null) => {
      ...
      const incoming = Array.from(list).filter((f) =>
        /\.(jpe?g|png|webp)$/i.test(f.name) || /^image\/(jpeg|png|webp)/.test(f.type),
      );
      ...
      // Defer so itemsRef reflects the new queue before the worker scans it.
  ...
  if (!supported) {
    return (
      <div className="rounded-[var(--radius-lg)] border border-mist bg-surface-2 p-6 text-center text-stone">
        Your browser can't run the on-device AI model. Please try a recent
        version of Chrome, Edge, or Firefox on a desktop for the best results.
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      ...
      <div
        role="button"
        tabIndex={0}
        aria-label="Add an image to remove its background"
      ...
      >
        ...
        <p className={["font-display text-ink", hasItems ? "text-base" : "text-xl"].join(" ")}>
          {hasItems ? "Drop another image" : "Drop your image here"}
        </p>
        {!hasItems && (
          <p className="mt-1 text-sm text-stone">
            or click to choose — the background is removed on your device
          </p>
        )}
      </div>
      ...
      {!modelReady && (
        <p className="mt-2 text-center text-xs text-faint">
          First use downloads a one-time AI model (~110&nbsp;MB), then it's cached
          and works offline.
        </p>
      )}

      {/* shared model-download banner */}
      {phase && (
      ...
      <div className="min-w-0 flex-1">
        ...
        {status === "working" && (
          <div className="mt-1.5">
            <p className="text-xs text-stone">Removing background on your device…</p>
